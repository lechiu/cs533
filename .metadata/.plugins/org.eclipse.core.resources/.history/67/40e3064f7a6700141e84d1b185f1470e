...
public class StartPoint extends StoryLineNode
{
  ...
  public StartPoint(Position aPosition, BlackCircle aBlackCircle)
  {
    super(aPosition);
    inBindings = new ArrayList<InBinding>();
    if (!setBlackCircle(aBlackCircle))
    {
      throw new RuntimeException("Unable to create StartPoint due to aBlackCircle");
    }
  }
  ...
  public BlackCircle getBlackCircle()
  {
    return blackCircle;
  }
  ...
  public boolean setBlackCircle(BlackCircle aNewBlackCircle)
  {
    boolean wasSet = false;
    if (aNewBlackCircle != null)
    {
      blackCircle = aNewBlackCircle;
      wasSet = true;
    }
    return wasSet;
  }
